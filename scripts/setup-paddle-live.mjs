/**
 * Provision live (or sandbox) Paddle catalog + webhook for Memory Recap.
 *
...
async function main() {
  ...
  let clientToken = tokens.find(
    (t) => t.status === "active" && t.name === "Memory Recap Web"
  );
  if (!clientToken) {
    clientToken = await paddle.clientTokens.create({
      name: "Memory Recap Web",
      description: "Browser checkout for Memory Recap",
    });
  ...
  if (!webhook) {
    webhook = await paddle.notificationSettings.create({
      description: "Memory Recap live billing",
