...
import { EARN_ENABLED } from "@/lib/flags";
...
export async function AppHeader({
  ...
  const navLinks = user ? (
    <>
      ...
      {EARN_ENABLED ? (
        <Link href="/moments" className={linkClass("moments")}>
          Earn
        </Link>
      ) : null}
    ...
    <>
      ...
      <Link href="/examples" className="min-h-11 px-2 text-sm text-neutral-500">
        Examples
      </Link>
      <Link href="/support" className="min-h-11 px-2 text-sm text-neutral-500">
        Help
      ...
      <Link
        href="/upload"
        className="min-h-11 rounded-xl bg-green-700 px-3 text-sm font-medium text-white"
      >
        Start
      </Link>
    </>
  ...
  return (
    <header className="sticky top-0 z-50 -mx-6 mb-2 border-b border-neutral-100 bg-white/95 px-6 py-3 backdrop-blur supports-[backdrop-filter]:bg-white/80">
